...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="ibox float-e-margins">
        <div class="ibox-title">
            <h5>配置问题类型：${feedbackType.text}</h5>
        </div>
...
<script>
    ...
    function initTable() {
        ...
        $("#cusTable").bootstrapTable({
            ...
            url: "${contextPath}/BG/feedbackProblem/list?feedbackTypeId=${feedbackType.id}", //获取数据的地址
            striped: true,  //表格显示条纹
            ...
            columns: [{
                ...
                field: 'id',
                formatter: function (row) {
                    return "<a  class='btn btn-info feedback-edit'  data-id='"+ row +"'>修改</a> <a class='btn btn-warning' onclick='articleDel(\""+row+"\")'>删除</a>"
    ...
    function feedbackadd() {
        $('.btn-feedback-add').on('click', function () {
            layer.prompt({
                ...
                $.ajax({
                    url:"${contextPath}/BG/feedbackProblem/edit?text="+value+"&feedbackTypeId=${feedbackType.id}",
                    dataType:"text",
    ...
    function feedbackEdit(){
        $('.feedback-edit').on('click', function () {
            ...
            layer.prompt({
                ...
                $.ajax({
                    url:"${contextPath}/BG/feedbackProblem/edit?text="+value+"&feedbackProblemId="+id+"&feedbackTypeId=${feedbackType.id}",
                    dataType:"text",
                    success:function (data) {
                        if(data=="success"){
                            initTable();
    ...
    function articleDel(id){
        layer.confirm('确认删除此记录?', {icon: 3, title: '提示'}, function (index) {
            //do something
            $.ajax({
                url:"${contextPath}/BG/feedbackProblem/delete",
                dataType:"text",
                data:{
                    "feedbackProblemId":id
                },
                success:function (data) {
                    if(data=="success"){
                        initTable();
